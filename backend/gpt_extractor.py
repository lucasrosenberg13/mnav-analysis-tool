...
# Ticker configuration - centralized and clean
TICKER_CONFIG = {
    'SBET': {
...
def get_latest_8k_url(cik: str) -> Optional[Tuple[str, str, str]]:
    ...
    Returns (url, accession_number, filing_date) or None
    """
    ...
    try:
        ...
        for form, acc, doc, date in zip(forms, accession_numbers, primary_docs, filing_dates):
            if form == "8-K":
                ...
                # Check if this filing contains Item 8.01
...
def extract_section_text(filing_url: str) -> str:
    """Extract the relevant section text from a filing"""
    try:
        ...
        if match:
            ...
            # Return first 12000 characters if no specific section found
...
def extract_data_with_gpt(filing_text: str, ticker: str) -> Tuple[Optional[int], Optional[int]]:
    """Use GPT to extract shares and crypto holdings from filing text"""
...
def get_company_data(ticker: str) -> Optional[Tuple[int, int, str]]:
    """Get company data: (total_shares, total_crypto, last_updated)"""
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
...
def get_last_processed_filing(ticker: str) -> Optional[str]:
    """Get last processed filing accession number"""
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
...
def add_processed_filing(ticker: str, accession_number: str, filing_date: str, 
    ...
    """Add processed filing to database"""
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute('''
...
def check_and_process_new_filings(ticker: str) -> Tuple[int, int]:
    """
    Main filing processing function using GPT
    Returns (crypto_holdings, total_diluted_shares)
    """
    ...
    print(f"[INFO] Current state: {current_total_shares:,} shares, {current_crypto_holdings:,} crypto, last filing: {last_processed_accession}")
    ...
    print(f"[INFO] Processing complete: {current_crypto_holdings:,} crypto, {current_total_shares:,} shares")
...
async def initialize_ticker(ticker: str, request: InitializeRequest):
    """Initialize a ticker with base data"""
    ticker = ticker.upper()
...
async def analyze_ticker(ticker: str):
    ...
    try:
        ...
        # Process filings and get current data
...
async def get_ticker_status(ticker: str):
    """Get current status for a ticker"""
    ticker = ticker.upper()
